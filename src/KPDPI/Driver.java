...
public class Driver {
	...
	public Driver(VertexPair s, int t) {
		subject = s;
		trajectories = t;
	}
	...
	public double LKDPI(VertexPair p){
		double ret = -11.30;
		if(p.getPatientAge() > 50)
			ret += 1.85*(p.getPatientAge()-50);
		ret -= 0.381*p.geteGFR();
		double bmi = p.getPatientBMI();
		if(p.isAfricanAmerican())
			bmi += 22.34;
		if(p.isCigaretteUser())
			bmi += 14.33;
		ret += 1.17*bmi;
		double sbp = p.getPatientSBP();
						...
						if (Cycle.isAChain(ci, pool)) {
							...
							for(Edge e : le){
								if(pool.getEdgeTarget(e).equals(subject)){
									ret[i][0] = LKDPI((VertexPair)pool.getEdgeTarget(e), (VertexAltruist)pool.getEdgeSource(e));
									ret[i][1] = t;
									stop = true;
									break;
								}
								...
								if (rDeparture.nextDouble() <= RENEGE) {
									...
									VertexAltruist bridgeDonor = new VertexAltruist(bridge.getID(),
											bridge.getBloodTypeDonor(), bridge.getDonorWeight(), bridge.isDonorMale(), bridge.getDonorHLA_B(), bridge.getDonorHLA_DR());
								...
								for(Edge e : ci.getEdges()){
									if(pool.getEdgeTarget(e).equals(subject)){
										...
										VertexAltruist donor = new VertexAltruist(donorPair.getID(),
												donorPair.getBloodTypeDonor(), donorPair.getDonorWeight(), donorPair.isDonorMale(), donorPair.getDonorHLA_B(), donorPair.getDonorHLA_DR());
